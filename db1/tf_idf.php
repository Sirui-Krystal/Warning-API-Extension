...
// reference pages
if ($part[3]=="reference"){
    foreach ($xpath->query('//*[@id="jd-content"]//code/a') as $div1){
        $t = $div1->textContent;
        array_push($mulEN,$t);
    }
...
// other pages
    ...
    foreach($xpath->query('//div[@class="jd-descr "]//code/a') as $div2){
        $t = $div2->textContent;
        array_push($mulEN,$t);
        /*
           if (!in_array($t,$a)){
...
//$test = "Intent is intent";
//$word = "intent";
//$c = substr_count(strtolower($test),$word);
$data_ranking = array();


foreach($data as $entityName=>$values){
    $data_ranking[$entityName] = array();
    $idf = log10($doc_num/$hasWarnEnName_warnNum[$entityName]);
    foreach($values as $class=>$sents){
        $ranking_sents = array();
        if (!array_key_exists($class, $data_ranking[$entityName])){
            $data_ranking[$entityName][$class] = array();
        }
        foreach($sents as $index=>$v){
            foreach ($v as $sent=>$url){
                $tf = substr_count(strtolower($sent),strtolower($entityName));
                $ranking = (int)$tf*$idf;
                $ranking_sents[$sent] = $ranking;
            }

        }
        arsort($ranking_sents);

        foreach($ranking_sents as $order_sent => $order_rank){
            array_push($data_ranking[$entityName][$class],$order_sent);
            $data_ranking[$entityName][$class][$order_sent] = $order_rank;
        }
    }
}
